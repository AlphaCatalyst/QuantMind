...
| QM2-KI-035 | medium | resolved | QM2-P0-010 implements the persistent content-addressed Store and migrates the complete current reachable research graph; runtime consumers still require the explicit QM2-P0-011 cutover. |
| QM2-KI-037 | medium | open | Artifact Store v1 is a single-host filesystem backend; runtime producers/consumers are not yet cut over, and NFS, cloud object storage, distributed locking and deletion GC are deliberately unsupported. |
