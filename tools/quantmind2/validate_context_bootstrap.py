...
def validate_bootstrap(root: Path = ROOT) -> list[str]:
    ...
    if not LEDGER_REFERENCE_ORM_CONTRACT.is_file():
        raise ValidationError("Ledger reference ORM contract is missing")
    reference_contract_text = LEDGER_REFERENCE_ORM_CONTRACT.read_text(encoding="utf-8")
    required_reference_contract_markers = {
        "Why Composite Primary Keys",
        "ComponentReference Mapping",
        "ArchitectureDecisionReference Mapping",
        "Why No Component or ADR Foreign Key",
        "Deferred Annotation Tables",
        "QM2-P0-002A2a2b2",
    }
    missing_reference_contract_markers = sorted(
        marker
        for marker in required_reference_contract_markers
        if marker not in reference_contract_text
    )
    if missing_reference_contract_markers:
        raise ValidationError(
            "Ledger reference ORM contract is missing markers: "
            f"{missing_reference_contract_markers}"
        )
    reference_orm_source = (LEDGER_ORM_ROOT / "orm_reference_models.py").read_text(
        encoding="utf-8"
    )
    required_reference_source_markers = {
        "from backend.services.api.models.base import Base",
    ...
    missing_reference_source = sorted(
        marker
        for marker in required_reference_source_markers
        if marker not in reference_orm_source
    )
    if missing_reference_source:
        raise ValidationError(
            f"Ledger reference ORM mapping is missing markers: {missing_reference_source}"
        )
    if any(marker in reference_orm_source for marker in forbidden_runtime_markers):
        raise ValidationError(
    ...
    if not mapper_root.is_dir() or {
        ...
        raise ValidationError("Ledger Mapper package does not match the complete v1 inventory")
    mapper_source = "\n".join(
        path.read_text(encoding="utf-8") for path in mapper_root.glob("*.py")
    )
    required_task_mapper_markers = {
        'MAPPER_CONTRACT_VERSION = "1.0.0"',
        "class RecordToDomainError",
        "class DomainToRecordError",
        "def enum_to_storage",
        ...
        "def run_relationship_to_record",
        "def run_relationship_from_record",
        "def changed_file_record_id",
        "def changed_symbol_record_id",
        "def changed_file_to_record",
        "def changed_file_from_record",
        "def changed_symbol_to_record",
        "def changed_symbol_from_record",
        "def test_execution_to_record",
        "def test_execution_from_record",
    ...
    unexpected_mapper_markers = sorted(
        marker for marker in forbidden_mapper_markers if marker in mapper_source
    )
    if unexpected_mapper_markers:
        raise ValidationError(
            f"Ledger Mapper package contains deferred runtime behavior: {unexpected_mapper_markers}"
        )
    if not LEDGER_TASK_MAPPER_CONTRACT.is_file():
        raise ValidationError("Ledger Task Mapper contract is missing")
    task_mapper_contract = LEDGER_TASK_MAPPER_CONTRACT.read_text(encoding="utf-8")
    required_task_contract_markers = {
        "Mapper Contract Version",
        "Mapper Error Model",
        "Enum Conversion",
        "Datetime Conversion",
        "JSON and Tuple Conversion",
    ...
    missing_task_contract_markers = sorted(
        marker
        for marker in required_task_contract_markers
        if marker not in task_mapper_contract
    )
    ...
    if not LEDGER_COMPLETE_MAPPER_CONTRACT.is_file():
        raise ValidationError("Ledger complete Mapper layer contract is missing")
    complete_mapper_text = LEDGER_COMPLETE_MAPPER_CONTRACT.read_text(encoding="utf-8")
    required_complete_mapper_markers = {
        "Completed Mapper Inventory",
        "Stable Technical IDs",
        "Parent Run Context",
        "Round-trip Semantics",
        "Mapper Drift Verification",
        "Deferred Repository Semantics",
        "Deferred Manifest Binding",
        "Deferred Migration",
        "QM2-P0-002A2b",
    }
    missing_complete_markers = sorted(
        marker for marker in required_complete_mapper_markers
        if marker not in complete_mapper_text
    )
    if missing_complete_markers:
        raise ValidationError(
            f"Ledger complete Mapper contract is missing markers: {missing_complete_markers}"
        )
    ...
    if handoff["current_task"] != "QM2-R2-010":
        raise ValidationError("machine handoff does not name current QM2-R2-010 task")
    if handoff["completion_status"] != "partial_uncommitted":
        raise ValidationError("machine handoff does not preserve the partial R2-010 result")
    if handoff["next_recommended_tasks"]:
        raise ValidationError("machine handoff must not invent an unauthorized successor")
    for fact in (
        "QM2-R2-002", "afc2_f3ba310a", "completed_no_holdout_survivor",
        "locked-holdout", "exact replay",
    ):
        if fact not in handoff_text:
            raise ValidationError(f"human handoff omits autonomous Campaign result: {fact}")
    for fact in (
        "QM2-R2-003", "afrp1_5ac295d3", "completed_no_validation_survivor",
        "15 total Rounds", "median RankIC", "BH FDR",
    ):
        if fact not in handoff_text:
            raise ValidationError(f"human handoff omits multi-Campaign Program result: {fact}")
    for fact in (
        "QM2-R2-004", "tfc2_229f19", "aap1_30e59b",
        "aausl1_47e182", "amtc1_fae10b", "zero Validation Survivor",
    ):
        if fact not in handoff_text:
            raise ValidationError(f"human handoff omits technical feature Program result: {fact}")
    for fact in (
        "QM2-R2-005", "2026-07-23", "retrospective_research_only",
        "peel1_f96c3595", "arsv1_1d07cf5c", "arsr1_3cc6becd",
    ):
        if fact not in handoff_text:
            raise ValidationError(f"human handoff omits Supervisor result: {fact}")
    for fact in (
        "QM2-R2-006", "tfc3_b903c85e", "arc2_78b9b6c",
        "18 Alpha Agent rounds", "68 formal Qlib calls",
    ):
        if fact not in handoff_text:
            raise ValidationError(
                f"human handoff omits technical-space Cycle result: {fact}"
            )
    for fact in (
        "QM2-R2-007", "fcms1_27c710", "mmtc1_659e9d",
        "54 LightGBM", "18 formal Qlib", "Cycle 004",
    ):
        if fact not in handoff_text:
            raise ValidationError(
                f"human handoff omits fixed-model Cycle result: {fact}"
            )
    for fact in (
        "QM2-R1-008", "40 formal Qlib calls", "stock_selection_signal_only",
        "zero Candidate Locks", "34.54%", "33.21%",
    ):
        if fact not in handoff_text:
            raise ValidationError(f"human handoff omits low-frequency result: {fact}")
    for fact in (
        "QM2-R1-009", "regime_specific_factor",
        "retain_for_regime_specific_research", "mtad1_308b2628",
        "No successor task",
    ):
        if fact not in handoff_text:
            raise ValidationError(f"human handoff omits tail-alpha result: {fact}")
    for fact in (
        "QM2-R1-002", "mfi1_87177b7c", "revision 2", "round 5",
        "Eight", "two-family", "no locks", "No successor task",
    ):
        if fact not in handoff_text:
            raise ValidationError(f"human handoff omits momentum result: {fact}")
    for fact in (
    ...
    for fact in (
        "htf_4a0762a5", "SecurityTerminationPolicyV1", "2025-07-11",
        "2025-04-21", "noncanonical", "Corporate Action Provider",
    ):
        if fact not in handoff_text:
