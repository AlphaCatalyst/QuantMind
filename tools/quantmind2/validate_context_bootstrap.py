...
def validate_bootstrap(root: Path = ROOT) -> list[str]:
    ...
    if handoff["current_task"] != "QM2-R2-009":
        raise ValidationError("machine handoff does not name current QM2-R2-009 task")
    if handoff["completion_status"] != "completed_uncommitted":
        raise ValidationError("machine handoff does not preserve the completed R2-005 result")
