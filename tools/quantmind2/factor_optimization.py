...
from backend.services.engine.artifact_runtime import inspect_optimization_replay, replay_optimization_study
...
def main(argv=None):
    ...
    inspect = sub.add_parser("inspect"); inspect.add_argument("--optimization-root"); inspect.add_argument("--study-id", required=True)
    replay = sub.add_parser("replay"); replay.add_argument("--study-id", required=True)
    args = parser.parse_args(argv)
    try:
        runtime = runtime_context_from_args(args)
        if args.command in {"inspect", "replay"} and runtime.policy.mode is not ArtifactRuntimeMode.LEGACY_LOCAL:
            result = inspect_optimization_replay(replay_optimization_study(runtime, args.study_id))
        elif args.command == "inspect":
            if not args.optimization_root: raise ValueError("legacy_local inspect requires --optimization-root")
            ...
            raise ValueError("legacy_local replay requires the inspect command and explicit root")
        else:
            if runtime.policy.mode is ArtifactRuntimeMode.STORE_REQUIRED:
                raise LegacyArtifactPathForbidden("store_required accepts Study IDs, not local Optimization inputs")
