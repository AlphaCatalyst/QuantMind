...
from backend.services.engine.autonomous_factor_program import (  # noqa: E402
...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="QuantMind autonomous multi-lane factor Program")
    value.add_argument("--store-root", type=Path)
    value.add_argument("--work-root", type=Path, default=Path("/private/tmp/qm2-r2-003"))
    commands = value.add_subparsers(dest="command", required=True)
    commands.add_parser("create-spec")
    validate = commands.add_parser("validate-spec")
    validate.add_argument("program_id")
    for name in (
        "plan", "execute-program", "resume-program", "pause-program",
        "inspect-program", "validate-program", "replay-program",
    ):
        command = commands.add_parser(name)
        command.add_argument("--program-id", required=True)
    for name in (
        "inspect-lane", "inspect-lane-memory", "inspect-lane-shortlist",
        "inspect-global-novelty", "inspect-search-exposure",
        "inspect-union-shortlist", "inspect-validation",
        "inspect-multiple-testing", "inspect-survivor", "inspect-near-miss",
        "inspect-fresh-lock",
    ):
        command = commands.add_parser(name)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "create-spec":
            result = create_program_spec(**common)
        ...
        elif args.command == "inspect-program":
            result = inspect_program(program_id=args.program_id, **common)
        ...
        elif args.command == "pause-program":
            result = {"status": "pause_requested_after_current_round", "program_id": args.program_id}
        elif args.command == "plan":
            state = inspect_program(program_id=args.program_id, **common)
            result = {
                "status": "planned", "next_lane": min(
                    state["lanes"].values(), key=lambda lane: (lane["rounds"], lane["lane_id"])
                )["lane_id"],
            }
        else:
