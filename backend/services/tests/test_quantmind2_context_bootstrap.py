...
class QuantMind2ContextBootstrapTests(unittest.TestCase):
    ...
    def test_handoff_records_external_campaign_and_exact_validation_next_task(self):
        text = (QM2 / "context" / "HANDOFF.md").read_text(encoding="utf-8")
        self.assertIn("candidates, approved and active are all zero", text)
        self.assertIn(
            "QM2-P0-008F — Agent Provider Enablement and External Campaign Completion",
            text,
        )
        handoff = load_json(QM2 / "context" / "handoff.json")
        self.assertEqual(handoff["current_task"], "QM2-P0-008F")
