...
def test_empty_cache_store_only_recovery_and_research_state(tmp_path):
    ...
    assert state.campaign_ids == tuple(sorted((ARTIFACTS[5][1], ARTIFACTS[6][1])))
    ...
    assert state.fresh_admission_result_id == ARTIFACTS[7][1]
...
def test_exact_existing_write_through_changes_no_store_identity(tmp_path):
    ...
    assert before.artifact_count == after.artifact_count == 65
    assert before.blob_count == after.blob_count == 280
